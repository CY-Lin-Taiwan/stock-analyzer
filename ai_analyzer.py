...
13. **技術位階必須提及**:若下方有「近期技術位階」區塊,你必須:
    a. 在 `qualitative_summary` 或 `signals_to_monitor` 中明確提到「目前布林位階 X%」
    b. 說明此位階對「近期短線觀察」的意義(例如:中軌之上偏強 / 靠近上軌警覺 / 跌破下軌反彈訊號等)
    c. 若你認為布林位階不重要(例如該股屬於長線存股),也要明說「布林位階雖在 X%,但對你的長線判斷影響不大,因為...」
    d. 不允許「完全不提布林位階」
...
{holder_json}
  ...
  "data_references": [
    "本次引用的關鍵數據點 1", "數據點 2",
    "(必須項) 布林位階引用,格式: '布林位階 X% (位階描述),代表近期...'"
  ],
...
11. data_references 必須包含「布林位階引用」(若有提供布林資料),格式: "布林位階 X% (位階描述)"
{holder_rule}
"""
    return prompt
